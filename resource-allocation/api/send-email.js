...
/* ─────────────────────────────────────────────────
   ...
   Uses selected workdayHours (6, 7, or 8)
   hoursPerDay × 5 days × 60 min = weekly capacity
───────────────────────────────────────────────── */
function workingMinutesInRange(startDateStr, endDateStr, workdayHours) {
  const start = new Date(startDateStr + 'T00:00:00');
  const end   = new Date(endDateStr   + 'T00:00:00');
  const msPerDay  = 86400000;
  const totalDays = Math.round((end - start) / msPerDay) + 1;
  const weeks     = Math.ceil(totalDays / 7);
  return weeks * workdayHours * 5 * 60; // hoursPerDay × 5 days × 60 min
}

/**
 * Compute base hours for email footer note.
 * weekBase  = workdayHours × 5
 * monthBase = workdayHours × 5 × (52/12) rounded
 */
function getBaseHours(workdayHours) {
  const weekBase  = workdayHours * 5;
  const monthBase = Math.round(workdayHours * 5 * (52 / 12));
...
/* ─────────────────────────────────────────────────
   BUILD EMAIL HTML
───────────────────────────────────────────────── */
function buildEmail(leader, aggWeek, aggMonth, wkRange, moRange, threshold, workdayHours) {
  const projects = leader.projects;
  const { weekBase, monthBase } = getBaseHours(workdayHours);
  ...
  const availWk = workingMinutesInRange(wkRange.startDate, wkRange.endDate, workdayHours);
  const availMo = workingMinutesInRange(moRange.startDate, moRange.endDate, workdayHours);
...
<table width="660" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:10px;overflow:hidden;border:1px solid #e0e0e0;">
  ...
  <tr><td style="padding:14px 28px 8px;">
    ...
    <p style="margin:4px 0 0;font-size:10px;color:#aaa;">
      Workday: <strong>${workdayHours}h/day</strong> &nbsp;|&nbsp;
      Week base = <strong>${weekBase}h</strong> &nbsp;|&nbsp;
      Month base = <strong>${monthBase}h</strong>
    </p>
  </td></tr>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Read workdayHours from saved config, default to 8
    const workdayHours = [6, 7, 8].includes(config.workdayHours) ? config.workdayHours : 8;
    ...
    for (const leader of leaders) {
      ...
      const html = buildEmail(
        leader, aggWeek, aggMonth,
        wkRange, moRange,
        threshold, workdayHours   // pass workdayHours to email builder
      );
